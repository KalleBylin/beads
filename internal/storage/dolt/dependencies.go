...
func (s *DoltStore) IsBlocked(ctx context.Context, issueID string) (bool, []string, error) {
	...
	rows, err := s.queryContext(ctx, `
		SELECT d.depends_on_id, d.type, COALESCE(i.status, ''), COALESCE(i.close_reason, '')
		FROM dependencies d
	...
	var blockers []string
	for rows.Next() {
		var id, depType, blockerStatus, closeReason string
		if err := rows.Scan(&id, &depType, &blockerStatus, &closeReason); err != nil {
			_ = rows.Close()
		...
		case string(types.DepBlocks), string(types.DepWaitsFor):
			blocksNow = blockerStatus != "" &&
				blockerStatus != string(types.StatusClosed) &&
				blockerStatus != string(types.StatusPinned)
		case string(types.DepConditionalBlocks):
			blocksNow = types.ConditionalBlocksReadyBlocked(types.Status(blockerStatus), closeReason)
		}
		if !blocksNow {
			continue
		}
...
func (s *DoltStore) GetNewlyUnblockedByClose(ctx context.Context, closedIssueID string) ([]*types.Issue, error) {
	closedIssue, err := s.GetIssue(ctx, closedIssueID)
	if err != nil {
		return nil, fmt.Errorf("failed to load closed issue: %w", err)
	}
	closedIssueFailed := closedIssue != nil &&
		closedIssue.Status == types.StatusClosed &&
		types.IsFailureClose(closedIssue.CloseReason)
	...
	for candidateRows.Next() {
		...
		if err := candidateRows.Scan(&id, &depType); err != nil {
			_ = candidateRows.Close()
			return nil, fmt.Errorf("failed to scan candidate: %w", err)
		}
		if depType == string(types.DepConditionalBlocks) && !closedIssueFailed {
			continue
		}
	...
	// Step 2: Filter to candidates that are no longer blocked according to the
	// authoritative readiness calculation.
	var unblockedIDs []string
	for _, id := range candidateIDs {
		blocked, _, err := s.IsBlocked(ctx, id)
		if err != nil {
			return nil, fmt.Errorf("failed to check blocked status for %s: %w", id, err)
		}
		if !blocked {
